...
class Zend_Form implements Iterator, Countable, Zend_Validate_Interface {
	...
	public function setDefaults(array $defaults) {
		...
		foreach ($this->getElements() as $name => $element) {
			...
			if (array_key_exists($name, $check)) {
				...
				$defaults = $this->_dissolveArrayUnsetKey($defaults,
						$belongsTo, $name);
			}
		}
			...
			if ($this->$name instanceof Zend_Form_Element
				...
				foreach ($this->$name->getElements() as $element) {
					...
					$suborder = (null !== $suborder) ? $suborder : count(
							$subordered);
					array_splice($subordered, $suborder, 0,
							array(
									$element
							));
				}
	...
	public function loadDefaultDecorators() {
		...
		if (empty($decorators)) {
			$this->addDecorator('FormElements')->addDecorator('HtmlTag',
					array(
							'tag' => 'dl',
							'class' => 'zend_form'
					))->addDecorator('Form');
