...
class Zend_Currency {
	...
	public function __construct($options = null, $locale = null) {
		...
		if (($this->_options['currency'] === null)
				and ($this->_options['name'] === null)) {
			...
			switch ($name) {
				...
				case 'format':
					if ((empty($value) === false)
							and (Zend_Locale::isLocale($value, null, false)
									=== false)) {
						if (!is_string($value)
				...
				case 'display':
					if (is_numeric($value) and ($value !== self::NO_SYMBOL)
							and ($value !== self::USE_SYMBOL)
							and ($value !== self::USE_SHORTNAME)
							and ($value !== self::USE_NAME)) {
						require_once 'Zend/Currency/Exception.php';
						throw new Zend_Currency_Exception(
