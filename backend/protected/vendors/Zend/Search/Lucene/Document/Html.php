...
class Zend_Search_Lucene_Document_Html extends Zend_Search_Lucene_Document {
		...
		foreach ($linkNodes as $linkNode) {
			if (($href = $linkNode->getAttribute('href')) != ''
					&& (!self::$_excludeNoFollowLinks
							|| strtolower($linkNode->getAttribute('rel'))
									!= 'nofollow')) {
				$this->_links[] = $href;
			}
		}
	...
	public static function loadHTML($data, $storeContent = false,
			$defaultEncoding = '') {
		return new Zend_Search_Lucene_Document_Html($data, false,
				$storeContent, $defaultEncoding);
	}
	...
	public function highlight($words, $colour = '#66ffff') {
		return $this->highlightExtended($words,
				array(
						$this,
						'applyColour'
				), array(
