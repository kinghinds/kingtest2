 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Search_Lucene_Storage_File_Memory extends
		Zend_Search_Lucene_Storage_File {
	/**
	 * FileData
	 *
	...
	public function readLong() {
		...
		if (PHP_INT_SIZE > 4) {
			...
			return ord($str[0]) << 56 | ord($str[1]) << 48 | ord($str[2]) << 40
					| ord($str[3]) << 32 | ord($str[4]) << 24
					| ord($str[5]) << 16 | ord($str[6]) << 8 | ord($str[7]);
		} else {
	...
	public function readLong32Bit() {
		...
		return $wordHigh * (float) 0x100000000/* 0x00000001 00000000 */ 
 + $wordLow;
	}
