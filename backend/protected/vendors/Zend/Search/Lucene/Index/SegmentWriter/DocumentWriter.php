 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Search_Lucene_Index_SegmentWriter_DocumentWriter extends
		Zend_Search_Lucene_Index_SegmentWriter {
	/**
	 * Term Dictionary
	...
	public function __construct(
			Zend_Search_Lucene_Storage_Directory $directory, $name) {
		...
		foreach ($document->getFieldNames() as $fieldName) {
			...
			if ($field->isIndexed) {
				if ($field->isTokenized) {
					...
					$term = new Zend_Search_Lucene_Index_Term($fieldUtf8Value,
							$field->name);
					$termKey = $term->key();
