...
class Zend_Search_Lucene_Index_SegmentInfo implements
	...
	public function __construct(
			Zend_Search_Lucene_Storage_Directory $directory, $name, $docCount,
			$delGen = 0, $docStoreOptions = null, $hasSingleNormFile = false,
			$isCompound = null) {
 ...
 && $tiVersion != (int) 0xFFFFFFFD /* 2.1+ format    */) {
		...
		$tisFile->seek(
				$prevTermInfo[4] /* indexPointer */ 
 - (($tiVersion == (int) 0xFFFFFFFD) ? 24 : 20) /* header size*/, SEEK_CUR);
		...
		for ($count = $prevPosition * $indexInterval + 1; $count <= $termCount
				&& ($this->_getFieldPosition($termFieldNum) < $searchDicField
						|| ($this->_getFieldPosition($termFieldNum)
								== $searchDicField
		...
		foreach ($this->_directory->fileList() as $file) {
			if ($file == $this->_name . '.del') {
				...
				$delFileList[] = 0;
					...
					'/^' . $this->_name . '_([a-zA-Z0-9]+)\.del$/i', $file,
					$matches)) {
 ...
 && $tiVersion != (int) 0xFFFFFFFD /* 2.1+ format    */) {
	...
	public function skipTo(Zend_Search_Lucene_Index_Term $prefix) {
		...
		$this->_lastTermInfo = new Zend_Search_Lucene_Index_TermInfo(
				$prevTermInfo[0] /* docFreq */, $prevTermInfo[1] /* freqPointer */,
				$prevTermInfo[2] /* proxPointer */, $prevTermInfo[3] /* skipOffset */);
		$this->_termCount = $this->_termNum
				- $prevPosition * $this->_indexInterval;
		
		if ($highIndex == 0) {
			// skip start entry
			$this->nextTerm();
		} else if ($prefix->field == $this->_lastTerm->field
			...
			if ($this->_termsScanMode == self::SM_FULL_INFO
				...
				$this->_frqFile->seek(
						$this->_lastTermInfo->freqPointer
								+ $this->_frqFileOffset, SEEK_SET);
				$freqs = array();
				$docId = 0;
				...
				$this->_prxFile->seek(
						$this->_lastTermInfo->proxPointer
								+ $this->_prxFileOffset, SEEK_SET);
				foreach ($freqs as $docId => $freq) {
					$termPosition = 0;
					$positions = array();
		...
		while ($this->_lastTerm !== null) {
			if (strcmp($this->_lastTerm->field, $prefix->field) > 0
					|| ($prefix->field == $this->_lastTerm->field
							&& strcmp($this->_lastTerm->text, $prefix->text)
									>= 0)) {
	...
	public function nextTerm() {
		...
		$termValue = Zend_Search_Lucene_Index_Term::getPrefix(
				$this->_lastTerm->text, $termPrefixLength) . $termSuffix;
		
		$this->_lastTerm = new Zend_Search_Lucene_Index_Term($termValue,
				$this->_fields[$termFieldNum]->name);
		
		$docFreq = $this->_tisFile->readVInt();
		$freqPointer = $this->_lastTermInfo->freqPointer
				+ $this->_tisFile->readVInt();
		$proxPointer = $this->_lastTermInfo->proxPointer
				+ $this->_tisFile->readVInt();
