...
class Zend_Search_Lucene_Index_Writer {
	...
	public function __construct(
			Zend_Search_Lucene_Storage_Directory $directory, &$segmentInfos,
			$targetFormatVersion) {
	...
	private function _updateSegments() {
		...
		try {
			...
			} else if ($this->_targetFormatVersion
					...
					if ($srcFormat == Zend_Search_Lucene::FORMAT_2_3) {
						...
						if ($docStoreOffset != (int) 0xFFFFFFFF) {
							...
							$docStoreOptions = array(
									...
									'isCompound' => ($docStoreIsCompoundFile
											== 1)
							);
						} else {
							$docStoreOptions = null;
		...
		if (Zend_Search_Lucene_LockManager::escalateReadLock($this->_directory)) {
			...
			foreach ($this->_directory->fileList() as $file) {
				if ($file == 'deletable') {
					...
					if ($file
						...
						$filesNumbers[] = (int) base_convert(substr($file, 9),
								36, 10); // ordered by segment generation numbers
					}
				} else if (preg_match('/(^_([a-zA-Z0-9]+))\.f\d+$/i', $file,
						$matches)) {
					...
					if (!isset($segments[$matches[1]])) {
						...
						$filesNumbers[] = (int) base_convert($matches[2], 36,
								10); // order by segment number
					}
				} else if (preg_match(
						'/(^_([a-zA-Z0-9]+))(_([a-zA-Z0-9]+))\.del$/i', $file,
						$matches)) {
					...
					if (!isset($segments[$matches[1]])) {
						...
						self::$_indexExtensions[substr($file, strlen($file) - 4)])) {
					...
					if (!isset($segments[$segmentName])
						...
						$filesNumbers[] = (int) base_convert(
								substr($file, 1 /* skip '_' */, strlen($file)
										- 5), 36, 10); // order by segment number
					}
				}
			}
			...
			foreach ($filesToDelete as $file) {
				try {
					...
					if (strpos($e->getMessage(), 'Can\'t delete file')
							=== false) {
