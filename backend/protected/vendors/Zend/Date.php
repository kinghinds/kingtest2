...
class Zend_Date extends Zend_Date_DateObject {
	...
	public function __construct($date = null, $part = null, $locale = null) {
		...
		if (($date !== null) and !is_array($date)
				and !($date instanceof Zend_TimeSync_Protocol)
				and !($date instanceof Zend_Date) and !defined($date)
				and Zend_Locale::isLocale($date, true, false)) {
			$locale = $date;
			$date = null;
			$part = null;
		} else if (($part !== null) and !defined($part)
				and Zend_Locale::isLocale($part, true, false)) {
			$locale = $part;
			$part = null;
		}
				...
				switch ($name) {
					...
					case 'extend_month':
						if (!is_bool($value)) {
							require_once 'Zend/Date/Exception.php';
							throw new Zend_Date_Exception(
									"'extend_month' has to be boolean", 0,
									null, $value);
						}
						break;
					case 'cache':
	...
	private function _parseIsoToDate($token, $locale) {
		switch ($token) {
			...
			case self::DATETIME:
				return $this->_toToken(
						Zend_Locale_Format::getDateTimeFormat($locale),
						$locale);
				break;
	...
	public function getTimezoneFromString($zone) {
		...
		if (!empty($match) and ($match[count($match) - 1] <= 12)
				and ($match[count($match) - 1] >= -12)) {
			$zone = "Etc/GMT";
	...
	private function _calculate($calc, $date, $part, $locale) {
		...
		switch ($part) {
			...
			case self::WEEKDAY_DIGIT:
				$weekday = (int) $this->toString(self::WEEKDAY_DIGIT, 'iso',
						$locale);
				if (is_numeric($date) and (intval($date) >= 0)
						and (intval($date) < 7)) {
					return $this->_assign($calc,
			...
			case self::MONTH_NAME:
				...
				if ($cnt < 12) {
					...
					return $this->_assign($calc,
							$this->mktime(0, 0, 0, $date, $day + $fixday,
									$year, true),
							$this->mktime(0, 0, 0, $month, $day, $year, true),
							$hour);
				}
			...
			case self::LEAPYEAR:
				require_once 'Zend/Date/Exception.php';
				throw new Zend_Date_Exception('leap year not supported', 0,
						null, $date);
				break;
			...
			case self::MERIDIEM:
				require_once 'Zend/Date/Exception.php';
				throw new Zend_Date_Exception('meridiem not supported', 0,
						null, $date);
				break;
			...
			case self::TIMEZONE_SECS:
				require_once 'Zend/Date/Exception.php';
				throw new Zend_Date_Exception('timezone not supported', 0,
						null, $date);
				break;
			
			case self::DAYLIGHT:
				require_once 'Zend/Date/Exception.php';
				throw new Zend_Date_Exception('daylight not supported', 0,
						null, $date);
				break;
			...
			case self::RFC_2822:
				...
				if (!$result) {
					require_once 'Zend/Date/Exception.php';
					throw new Zend_Date_Exception(
							"no RFC 2822 format ($date)", 0, null, $date);
				}
				...
				try {
					...
					return $this->_assign($calc,
							$this->mktime($parsed['hour'], $parsed['minute'],
									$parsed['second'], $month, $day, $year,
									true),
							$this->mktime($hour, $minute, $second, $month,
									$day, $year, true), false);
				} catch (Zend_Locale_Exception $e) {
					require_once 'Zend/Date/Exception.php';
					throw new Zend_Date_Exception($e->getMessage(), 0, $e,
				...
				if (!is_numeric($date) || !empty($part)) {
					try {
						...
						if ((strpos(strtoupper($part), 'YY') !== false)
								and (strpos(strtoupper($part), 'YYYY')
										=== false)) {
							$parsed['year'] = self::getFullYear($parsed['year']);
						}
	...
	private function _time($calc, $time, $format, $locale) {
		...
		if ($time instanceof Zend_Date) {
			...
			if (is_array($time)) {
				if ((isset($time['hour']) === true)
						or (isset($time['minute']) === true)
	...
	private function _checkLocation($location) {
		...
		if (($location['longitude'] > 180) or ($location['longitude'] < -180)) {
			require_once 'Zend/Date/Exception.php';
			throw new Zend_Date_Exception(
					'Longitude must be between -180 and 180', 0, null,
					$location);
		}
		if (($location['latitude'] > 90) or ($location['latitude'] < -90)) {
			require_once 'Zend/Date/Exception.php';
			throw new Zend_Date_Exception(
					'Latitude must be between -90 and 90', 0, null, $location);
		}
	...
	private function _calcvalue($calc, $value, $type, $parameter, $locale) {
		...
		} else if (!is_array($value) && !is_numeric($value) && ($type != 'iso')
				&& ($type != 'arpa')) {
			require_once 'Zend/Date/Exception.php';
			throw new Zend_Date_Exception("invalid $type ($value) operand", 0,
					null, $value);
		}
