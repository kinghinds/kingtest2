...
class Zend_Ldap {
	 ...
	 * not a valid DN.
	 */
	public static function explodeDn($dn, array &$keys = null,
			array &$vals = null) {
		/**
		 * @see Zend_Ldap_Dn
		 */
	 ...
	 * @return string
	 */
	public function getLastError(&$errorCode = null,
	 ...
	 * @throws Zend_Ldap_Exception
	 */
	public function connect($host = null, $port = null, $useSsl = null,
			$useStartTls = null) {
		if ($host === null) {
			$host = $this->_getHost();
		}
	...
	public function bind($username = null, $password = null) {
		...
		if ($username !== null && $password === ''
			...
			switch ($this->getLastErrorCode()) {
				case Zend_Ldap_Exception::LDAP_SERVER_DOWN: /* If the error is related to establishing a connection rather than binding,
	 ...
	 * @throws Zend_Ldap_Exception
	 */
	public function search($filter, $basedn = null,
			$scope = self::SEARCH_SCOPE_SUB, array $attributes = array(),
			$sort = null, $collectionClass = null, $sizelimit = 0,
			$timelimit = 0) {
		if (is_array($filter)) {
	 ...
	 * @throws Zend_Ldap_Exception
	 */
	public function count($filter, $basedn = null,
	...
	public function searchEntries($filter, $basedn = null,
			$scope = self::SEARCH_SCOPE_SUB, array $attributes = array(),
			$sort = null, $reverseSort = false, $sizelimit = 0, $timelimit = 0) {
		if (is_array($filter)) {
	...
	protected function _getChildrenDns($parentDn) {
		...
		$children = array();
		$search = @ldap_list($this->getResource(), $parentDn,
				'(objectClass=*)', array(
						'dn'
				));
		for ($entry = @ldap_first_entry($this->getResource(), $search); $entry
				!== false; $entry = @ldap_next_entry($this->getResource(),
				$entry)) {
	 ...
	 * @throws Zend_Ldap_Exception
	 */
	public function move($from, $to, $recursively = false,
			$alwaysEmulate = false) {
	 ...
	 * @throws Zend_Ldap_Exception
	 */
	public function rename($from, $to, $recursively = false,
			$alwaysEmulate = false) {
