...
class Zend_Pdf {
	...
	protected function _dumpPages() {
		...
		foreach ($this->_namedTargets as $name => $namedTarget) {
			if ($namedTarget instanceof Zend_Pdf_Destination_Explicit) {
				...
				require_once 'Zend/Pdf/Exception.php';
				throw new Zend_Pdf_Exception(
						'Wrong type of named targed (\''
								. get_class($namedTarget) . '\').');
			}
		}
		...
		$iterator = new RecursiveIteratorIterator(
				new Zend_Pdf_RecursivelyIteratableObjectsContainer(
						$this->outlines),
	...
	protected function _dumpOutlines() {
		...
		if ($root->Outlines === null) {
			...
			if (count($this->_originalOutlines) != count($this->outlines)) {
				...
				$updateOutlinesNavigation = true;
			} else if (!(array_keys($this->_originalOutlines)
	...
	public function setMetadata($metadata) {
		...
		$metadataObject->dictionary->Type = new Zend_Pdf_Element_Name(
				'Metadata');
	...
	protected function _cleanUpAction(Zend_Pdf_Action $action,
		...
		if ($action instanceof Zend_Pdf_Action_GoTo
				&& $this->resolveDestination($action->getDestination(), false)
						=== null) {
		...
		foreach ($iterator as $chainedAction) {
			if ($chainedAction instanceof Zend_Pdf_Action_GoTo
					&& $this->resolveDestination(
							$chainedAction->getDestination(), false) === null) {
		...
		if (!$this->_objFactory->isModified()) {
			...
			if ($outputStream === null) {
				...
				while (strlen($pdfData) > 0
						&& ($byteCount = fwrite($outputStream, $pdfData))
								!= false) {
			...
			if ($updateInfo->isFree()) {
				...
				if ($outputStream === null) {
					...
					while (strlen($pdfBlock) > 0
							&& ($byteCount = fwrite($outputStream, $pdfBlock))
									!= false) {
