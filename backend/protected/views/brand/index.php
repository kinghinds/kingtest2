...
function recommendFn(){
	$('#form').attr('action','/backend/index.php?r=brand/batchCommend');
	$('#form').removeAttr('onsubmit');
	$('#form').submit();
}
function unRecommendFn(){
	$('#form').attr('action','/backend/index.php?r=brand/batchUnCommend');
	$('#form').removeAttr('onsubmit');
	$('#form').submit();
}
//-->
...
<div class="heading">
	<h1><?php echo CHtml::image(Yii::app()->baseUrl . '/image/product.png'); ?>品牌</h1>
	<div class="buttons">
		<?php if (Yii::app()->user->checkAccess('createBrand')) { ?>
...
<div class="content">
	...
	<div class="clearfix">
		...
		<?php echo CHtml::form(array('batchDelete'), 'post', array('id'=>"form",
			'onsubmit' => 'batchDelete("确认要删除选中数据吗?", this, "brand_id[]"); return false;'
		)); ?>
		...
		<?php $this->widget('zii.widgets.grid.CGridView', array(
			...
			'columns' => array(
				...
				array('name' => 'image_path',
										'header' =>'缩略图',
										'value' => 'CHtml::image($data->getBrandFileUrl(),$data->brand_id,array("width"=>200,"height"=>100))',
										'type' => 'raw',
				...
				array(
					'name' => 'region_id',
					'value' => '$data->region!=null?$data->region->title:"未知"',
					'headerHtmlOptions' => array('style' => 'width:80px;'),
					'htmlOptions' => array('align' => 'center')
				),
				array(
					'class' => 'CButtonColumn',
					'header' => Brand::model()->getAttributeLabel('sort_order'),
					...
					'visible' => Yii::app()->user->checkAccess('updateBrand')
				),
				array(
					'name' => 'sort_order',
					'header' => Brand::model()->getAttributeLabel('sort_order') . '2',
				...
				array(
					...
					'buttons' => array(
						...
						'delete' => array(
							'visible' => 'Yii::app()->user->checkAccess("deleteBrand")'	
						)
					)					
				)
			)
		...
		<?php if (Yii::app()->user->checkAccess('deleteBrand')) { ?>
