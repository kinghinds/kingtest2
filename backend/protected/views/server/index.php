...
function recommendFn(){
	$('#form').attr('action','/backend/index.php?r=server/batchCommend');
	$('#form').removeAttr('onsubmit');
	$('#form').submit();
}
function unRecommendFn(){
	$('#form').attr('action','/backend/index.php?r=server/batchUnCommend');
	$('#form').removeAttr('onsubmit');
	$('#form').submit();
}
//-->
...
<div class="heading">
	<h1><?php echo CHtml::image(Yii::app()->baseUrl . '/image/product.png'); ?>服务</h1>
	<div class="buttons">
		<?php if (Yii::app()->user->checkAccess('createServer')) { ?>
...
<div class="content">
	...
	<div class="clearfix">
		...
		<?php echo CHtml::form(array('batchDelete'), 'post', array('id'=>"form",
			'onsubmit' => 'batchDelete("确认要删除选中数据吗?", this, "server_id[]"); return false;'
		)); ?>
		...
		<?php $this->widget('zii.widgets.grid.CGridView', array(
			...
			'columns' => array(
				...
				array('name' => 'image_path',
										'header' =>'缩略图',
										'value' => 'CHtml::image($data->getThumbFileUrl(),$data->server_id,array("width"=>200,"height"=>100))',
										'type' => 'raw',
				...
				array(
					'class' => 'CButtonColumn',
					'header' => Server::model()->getAttributeLabel('sort_order'),
					...
					'visible' => Yii::app()->user->checkAccess('updateServer')
				),
				array(
					'name' => 'sort_order',
					'header' => Server::model()->getAttributeLabel('sort_order') . '2',
				...
				array(
					...
					'buttons' => array(
						...
						'delete' => array(
							'visible' => 'Yii::app()->user->checkAccess("deleteServer")'	
						)
					)					
				)
			)
		...
		<?php if (Yii::app()->user->checkAccess('deleteServer')) { ?>
