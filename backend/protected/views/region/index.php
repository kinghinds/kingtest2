...
<div class="heading">
	<h1><?php echo CHtml::image(Yii::app()->baseUrl . '/image/product.png'); ?>品牌地区</h1>
	<div class="buttons">
		<?php if (Yii::app()->user->checkAccess('createRegion')) { ?>
...
<div class="content">
	...
	<div class="clearfix">
		...
		<?php echo CHtml::form(array('batchDelete'), 'post', array(
			'onsubmit' => 'batchDelete("确认要删除选中数据吗?", this, "region_id[]"); return false;'
		)); ?>
		...
		<?php $this->widget('zii.widgets.grid.CGridView', array(
			...
			'columns' => array(
				...
				array(
					'class' => 'CButtonColumn',
					'header' => Region::model()->getAttributeLabel('sort_order'),
					...
					'visible' => Yii::app()->user->checkAccess('updateRegion')
				),
				array(
					'name' => 'sort_order',
					'header' => Region::model()->getAttributeLabel('sort_order') . '2',
				...
				array(
					...
					'buttons' => array(
						...
						'delete' => array(
							'visible' => 'Yii::app()->user->checkAccess("deleteRegion")'	
						)
					)					
				)
			)
		...
		<?php if (Yii::app()->user->checkAccess('deleteRegion')) { ?>
