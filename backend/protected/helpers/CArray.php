...
class CArray {
	...
	public static function map_recursive($callback, array $array) {
		foreach ($array as $key => $val) {
			...
			$array[$key] = is_array($val) ? arr::map_recursive($callback, $val) : call_user_func(
					$callback, $val);
		}
	 ...
	 * @return  integer
	 */
	public static function binary_search($needle, $haystack, $nearest = FALSE,
			$sort = FALSE) {
		if ($sort === TRUE) {
			sort($haystack);
		}
		...
		if ($high == count($haystack) OR $haystack[$high] != $needle) {
			...
			return ($high_distance >= $low_distance) ? $haystack[ceil($low)] : $haystack[floor(
					$low)];
		}
